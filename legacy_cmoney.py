...
class Stock_requests():
    ...
    def _selected_account_id(self):
        if not self.Account_ID:
            raise RuntimeError("CMoney account name is not configured")
        try:
            return self.user_data[self.Account_ID]
        except KeyError as exc:
            raise RuntimeError("Configured CMoney account was not returned by the site") from exc
    ...
    def DeleteEntrust(self, order):
        """刪單"""
        user = self._selected_account_id()
    ...
    def get_ProfitLoss_accomplished(self):
        ...
        url = 'https://www.cmoney.tw/vt/ashx/accountdata.ashx?act=ProfitLoss&aid={}&profitLossType=unaccomplished'.format(self._selected_account_id())
        # user = input('選擇使用者帳戶:')
    ...
    def get_ProfitLoss_unaccomplished(self):
        ...
        url = 'https://www.cmoney.tw/vt/ashx/accountdata.ashx?act=ProfitLoss&aid={}&profitLossType=accomplished'.format(self._selected_account_id())
        # user = input('選擇使用者帳戶:')
        # lookday = int(input('查詢天數:'))
        # today = datetime.date.today()
        # startTime = today - datetime.timedelta(days=lookday)
    ...
    def get_GetProfitLossDetail_accomplished(self, order_no):
        """損益試算  已實現損益  取得明細"""
        print('-' * 50, '損益試算  已實現損益', '-' * 50)
        url = 'https://www.cmoney.tw/vt/ashx/accountdata.ashx?act=GetProfitLossDetail&profitLossType=accomplished&orderNo={}'.format(order_no)
...
if __name__ == '__main__':
    user_id = os.getenv("CMONEY_USER_ID")
    password = os.getenv("CMONEY_PASSWORD")
    account_name = os.getenv("CMONEY_ACCOUNT_NAME")

    if not user_id or not password or not account_name:
        raise SystemExit(
            "Set CMONEY_USER_ID, CMONEY_PASSWORD and CMONEY_ACCOUNT_NAME before running this historical adapter."
        )

    stock = Stock_requests(account_name=account_name)
    if not stock.login(user_id, password):
        raise SystemExit("CMoney login failed")
    stock.get_user_data()
